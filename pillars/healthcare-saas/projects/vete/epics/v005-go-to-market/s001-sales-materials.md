# S001: Sales Materials

**Epic:** [v005-go-to-market](_epic.md)  
**Status:** 📋 Ready  
**Effort:** ~3 hours

---

## User Story

**As a** founder  
**I want** professional sales materials  
**So that** I can effectively pitch Vete to clinic owners

---

## Acceptance Criteria

- [ ] 1-page PDF sales deck (Spanish)
- [ ] Product screenshots/demo video
- [ ] Pricing one-pager
- [ ] WhatsApp pitch template
- [ ] Email pitch template

---

## Tasks

| ID | Task | Effort | Status | Owner |
|----|------|--------|--------|-------|
| T001 | Create 1-page sales deck PDF | 1h | ⬜ | — |
| T002 | Capture product screenshots | 30m | ⬜ | — |
| T003 | Create pricing one-pager | 30m | ⬜ | — |
| T004 | Write WhatsApp pitch template | 30m | ⬜ | — |
| T005 | Write email pitch template | 30m | ⬜ | — |

---

## Sales Deck Outline

1. **El Problema**
   - Gestión manual = errores
   - WhatsApp caos
   - Sin datos para decisiones

2. **La Solución: Vete**
   - Todo en un lugar
   - Acceso desde cualquier dispositivo
   - Datos en tiempo real

3. **Funcionalidades**
   - Agenda y citas
   - Historias clínicas
   - Inventario y facturación
   - Portal para dueños

4. **Por Qué Vete**
   - Diseñado para Paraguay
   - Soporte local
   - Precio accesible

5. **Precio**
   - $50/mes por clínica
   - Prueba gratis 30 días
   - Sin contratos largos

6. **Siguiente Paso**
   - Agendar demo
   - Contacto
